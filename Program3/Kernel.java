		...
		case INTERRUPT_SOFTWARE: // System calls
			switch( cmd ) { 
				...
				case WAIT:
					...
					// let the current thread sleep in waitQueue under the condition = this thread id
					int childTID = waitQueue.enqueueAndSleep(currentTID);	

					return childTID; // return a child thread id who woke me up
				case EXIT:
					// get the current thread's parent id
					myTcb = scheduler.getMyTcb();
					int currentPID = myTcb.getPid();
					int currentTID2 = myTcb.getTid();
					...
					if (currentTID2 == 0) {
						...
						waitQueue.dequeueAndWakeup(currentPID, currentTID2);
					}
